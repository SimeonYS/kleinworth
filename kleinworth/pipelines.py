...
class KleinworthPipeline:

    # Database setup
    conn = sqlite3.connect('kleinworth.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `kleinworth`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM kleinworth WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO kleinworth (date, title, link, content)"
